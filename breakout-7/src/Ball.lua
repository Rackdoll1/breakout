...
function Ball:init(skin)
	...
	-- our table of Quads relating to the global block of texture using this
	self.skin = skin
end
...
function Ball:collides(target)
	if self.y + self.height >= target.y and self.y <= target.y + target.height then
...
function Ball:update(dt)
	...
	if self.x <= 0 then
		self.x = 0
		self.dx = -self.dx
		if gSounds['wall-hit']:isPlaying() then
