...
function LevelMaker.createMap(level)
	...
	for y = 1, numRows do
		...
		-- whether we want to enable skipping this row
		...
		-- used obly when we want to alternate a block, for alternate pattern
		...
		for x = 1, numCols do
			...
			if skipPattern and skipFlag then
				...
				-- as Lua doesn´t have a continue statement, we do...
				goto continue
			else
				-- flip the flag to true so next iteration makes the skip
				skipFlag = not skipFlag
			end
	...
	if #bricks == 0 then -- # unitary operator for length of string or table
