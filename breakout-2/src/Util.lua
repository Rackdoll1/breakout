...
function GenerateQuads(atlas, tilewidth, tileheight)
	-- these are for determining the number of loops we will compute
	local sheetWidth = atlas:getWidth() / tilewidth
	local sheetHeight = atlas:getHeight() / tileheight

	-- for indexing our table (lua´s tables start indexing from 1)
	local sheetCounter = 1
	local spritesheet = {}

	for y = 0, sheetHeight - 1 do
		for x = 0,  sheetWidth - 1 do
			spritesheet[sheetCounter] = love.graphics.newQuad(x * sheetWidth, y * sheetHeight,
				sheetWidth, sheetHeight, atlas:getDimensions())
...
function table.slice(tbl, first, last, step)
	...
	-- #tbl means the size of the table
...
function GenerateQuadsPaddles(atlas)
	local x = 0
	local y = 64 -- this accounts for the height of previous paddles we dont want in the sprite sheet

	local counter = 1
	local quads = {}

	-- we will only do it 4 times, for 4 paddles
	for i = 0, 3 do
		-- smallest
